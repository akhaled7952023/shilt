<div class="card">
    <div class="card-header">
        <h4 class="card-title">{{ __('dashboard.vehicle_timeline') }}</h4>
                    ...
                    <div class="mr-2 mt-1" style="min-width: 28px;">
                        @switch($entry->action)
                            ...
                            @case('status_changed')
                                ...
                                @break
                            @case('assigned')
                                <i class="la la-user text-primary font-medium-3"></i>
                                @break
                            @case('returned')
                            ...
                            @switch($entry->action)
                                @case('created')
                                    @switch($entry->model_type)
                                        ...
                                        @case('App\Models\VehicleDocument') تم رفع وثيقة @break
                                        @case('App\Models\VehicleAssignment') تم تعيين المركبة @break
                                        @case('App\Models\VehicleRental') تم إضافة بيانات إيجار @break
                                        @default تم الإنشاء
                                    @endswitch
                                    @break
                                @case('updated')
