@php
    ...
    $isSupportNotif = $n instanceof SupportNotification;
    ...
    if ($isSupportNotif) {
        ...
        $linkLabel = __('portal.view_ticket_link');
    } elseif ($n->type === 'settlement_published' && isset($n->data['period_id'])) {
        $link      = route('portal.settlements.show', $n->data['period_id']);
        $linkLabel = __('portal.view_settlement_link');
    }
@endphp

<div class="notif-item {{ $n->isUnread() ? 'unread' : '' }}"
     id="notif-{{ $isSupportNotif ? 's' : '' }}{{ $n->id }}">
    ...
    <div class="notif-content">
        ...
        @php $body = $n->getLocalizedBody(); @endphp
        @if($body)
        ...
        <div class="notif-meta">
            ...
            @if($link)
                <a href="{{ $link }}" style="color:var(--primary);font-weight:600;text-decoration:none;font-size:12px;">
                    <i class="la la-external-link" style="{{ $isRtl ? 'margin-left:2px;' : 'margin-right:2px;' }}"></i>{{ $linkLabel }}
                </a>
            @endif
        </div>
    </div>
